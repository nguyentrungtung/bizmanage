...
(function(window, document, undefined) {
	 ...
	 * 
	 * @param elementContainer - String - The name attribute of the form (i.e.
	...
	var jsValidator = function(elementContainer, fields, callback) {
		this.callback = callback || defaults.callback;
		this.errors = [];
		this.fields = {};

		elementContainer = $(elementContainer);
		this.container = (typeof elementContainer === 'object') ? elementContainer
				: {};
	...
	jsValidator.prototype.setMessage = function(rule, message) {
	...
	jsValidator.prototype.registerCallback = function(name, handler) {
	...
	jsValidator.prototype.registerConditional = function(name, conditional) {
	...
	jsValidator.prototype._addField = function(field, nameValue) {
	...
	jsValidator.prototype._validateForm = function() {
		this.errors = [];

		for ( var key in this.fields) {
			if (this.fields.hasOwnProperty(key)) {

				var field = this.fields[key] || {}, element = $(this.container
						.find("input[name='" + field.name
								+ "'], textarea[name='" + field.name + "']"));
				if (element && element !== undefined) {

					field.id = element.attr('id');
					field.element = element;
					field.type = (element.length > 0) ? element.attr('type')
							: '';

					if (element.is(':radio') || element.is(':checkbox')) {
						field.value = element.find(':checked').val();
					} else {
						field.value = element.val();
					}
					field.checked = element.is(':checked');
	...
	jsValidator.prototype._validateField = function(field) {
	...
	jsValidator.prototype._hooks = {
		...
		matches : function(field, matchName) {
			var el = this.container[matchName];
